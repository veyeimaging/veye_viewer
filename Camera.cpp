...
Camera::Camera(CommDevCfg devCfg, QObject *parent)
    : QObject{parent}
    , m_devCfg(devCfg)
{
    m_bThread = true;
...
void Camera::runSystemCmd(QString strCmd)
{
    int nLen = 1024;
    char szBuf[1024] = {0};
    m_mtxCmd.lock();
...
void Camera::asynReadData(StI2CCmd cmd)
{
    m_mtx.lock();
...
void Camera::asynWriteDate(StI2CCmd cmd)
{
    m_mtx.lock();
    m_listCmd.push_back(cmd);
    m_mtx.unlock();
...
void Camera::syncReadData(StI2CCmd &cmd)
{
    m_mtx.lock();
    readData(cmd);
    m_mtx.unlock();
...
int Camera::readData(StI2CCmd &cmd)
{
    int ret = QX_FAILURE;
    QString strMsg;
...
int Camera::writeData(StI2CCmd &cmd)
{
    int ret = -1;
    QString strMsg;
    QString strReg = QString("0x%1").arg(cmd.reg, 2, 16, QLatin1Char('0'));
...
void Camera::systemCmd(QString strCmd)
{
...
int Camera::system_ex(const char *cmdstring, unsigned int timeout)
{
    pid_t pid;
    int status;
...
int Camera::runSystem(const char *cmdstring, char *buff, int size)
{
...
void Camera::cmdSleep(int n, bool bReal)
{
    if (bReal) {
        QThread::msleep(n);
    }
