...
ExpGainDlg::ExpGainDlg(CommDevCfg devCfg, QWidget *parent)
    : QDialog(parent)
    , m_devCfg(devCfg)
...
void ExpGainDlg::setupUi()
{
    setFocusPolicy(Qt::NoFocus);

    if (EuCamType::GxCamera == m_devCfg.cameType) {
        ui->pushButtonOneAEAG->hide();
        ui->label_gain->hide();
        ui->comboBox_gain->hide();
    }
    ...
    ui->horizontalSlider_agmaxgain->setRange(0, 1000);
    ui->spinBox_agmaxgain->setRange(0.0, 100.0);
    ...
    ui->horizontalSlider_mgain->setRange(0, 1000);
    ui->spinBox_mgain->setRange(0.0, 100.0);
